...
class VisionTransformer(nn.Module):
    ...
    def forward(self, x: torch.Tensor, return_all=False, instance_masks=None):
        B, nc, w, h = x.shape
        ...
        if self.arch == 'vanilla':  # If attn_strategy is also 'vanilla', custom_attn acts the same as vanilla CLIP attn
            x = x + self.custom_attn(blk.attn, blk.ln_1(x), n_patches, instance_masks=instance_masks)
            x = x + blk.mlp(blk.ln_2(x))
        elif self.arch == 'reduced':
            x = self.custom_attn(blk.attn, blk.ln_1(x), n_patches, instance_masks=instance_masks)
        else:
    ...
    def custom_attn(self, attn_layer, x, n_patches, return_attn=False, with_attn=False, instance_masks=None):
        ...
        if self.attn_strategy in ['naclip', 'nonly']:
            ...
            if instance_masks is not None:
                num_tokens_sr = attn_weights.shape[-1]
                num_patches_sr = num_tokens_sr - 1  # exclude CLS
                num_heads_sr = attn_layer.num_heads
                B_sr = attn_weights.shape[0] // num_heads_sr

                # Use attn logits (avg over heads) for m_0 thresholding
                attn_for_thresh = attn_weights.view(B_sr, num_heads_sr, num_tokens_sr, num_tokens_sr)
                patch_sim = attn_for_thresh[:, :, 1:, 1:].mean(dim=1)  # (B, P, P)

                for b_idx in range(B_sr):
                    mask_flat = instance_masks[b_idx]  # (P,) int
                    E = torch.zeros(num_patches_sr, num_patches_sr,
                                    dtype=torch.bool, device=attn_weights.device)
                    unique_ids = torch.unique(mask_flat)

                    for uid in unique_ids:
                        m = (mask_flat == uid.item())
                        if uid.item() == 0:  # unsegmented region (m_0)
                            sim_b = patch_sim[b_idx]
                            E |= ((m.unsqueeze(0) | m.unsqueeze(1))
                                  & (sim_b > sim_b.mean()))
                        else:  # segmented region
                            E |= torch.outer(m, m)

                    # Apply to all heads for this batch item
                    h_start = b_idx * num_heads_sr
                    h_end = (b_idx + 1) * num_heads_sr
                    E_expanded = E.unsqueeze(0).expand(num_heads_sr, -1, -1)
                    attn_weights[h_start:h_end, 1:, 1:] = torch.where(
                        E_expanded,
                        attn_weights[h_start:h_end, 1:, 1:],
                        torch.tensor(float('-inf'), device=attn_weights.device, dtype=attn_weights.dtype)
                    )
            # === End SR ===
...
class CLIP(nn.Module):
    ...
    def encode_image(self, image, return_all=False, instance_masks=None):
        return self.visual(image.type(self.dtype), return_all=return_all, instance_masks=instance_masks)
